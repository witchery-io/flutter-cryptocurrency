main() {
  /* group('[BTC] transaction', () {
    test('[BTC] (negative price) (empty address)', () async {
      const mnemonic =
          'limit boost flip evil regret shy alert always shine cabin unique angry';
      final mc = MultiCurrency(mnemonic);

      final curr = await mc.getCurrencies;

      if (curr.first is BTC) {
      expect(curr.first.transaction('muoy6CytsecJmqrkpxsgSsarw1nGfEnuYs', -0.0002), throwsArgumentError);
      expect(curr.first.transaction('', 0.0002), throwsArgumentError);
      expect(curr.first.transaction('muoy6CytsecJmqrkpxsgSsarw1nGfEnuYs', 9999.00), throwsException);
      }
    });

    test('[BTC] Tx class', () async {
      final tx = Tx(
        senderAddress: 'mgpWpWc4dg5VaFqFqn2QDg5oBLCyh2oCXU',
        balance: 8148312,
        address: 'muoy6CytsecJmqrkpxsgSsarw1nGfEnuYs',
        price: 220000,
        ecPair: ECPair.fromWIF(
            'cQKqnYbYVMBgsUHHnPucb6QjRFZaw7MZBv89U5rN3kYodqUeXFJF'),
        outputs: [
          {
            'hash':
                'f6ce386202b7ef9e84629055b851be3f0c2135929271343afeb5ab14b450d361',
            'index': 0
          },
          {
            'hash':
                '638b0160ae86785d6ff2352f99fc9136c01f0dd646d253b1129362a238a1a972',
            'index': 1
          },
          {
            'hash':
                '51648f46f83c930f8236b57bb67b7614c48461b6a75099674de2d33c054f5739',
            'index': 0
          },
        ],
        fee: 100000.0,
      );

      tx.build();
      expect(tx.txHex,
          '');
    });
  });*/
}
